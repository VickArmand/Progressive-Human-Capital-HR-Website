jQuery(document).ready(function($) {
    'use strict';
    var this_obj = hr_management_consultancy_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.hr-management-consultancy-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message hr-management-consultancy-plugin-install');
                button.addClass('button-primary hr-management-consultancy-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.hr-management-consultancy-recommended-plugins .hr-management-consultancy-activation-note').css('display','block');
    ...
    $(document).on('click', '.hr-management-consultancy-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .hr-management-consultancy-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .hr-management-consultancy-recommended-plugins').css('display','none');
    });
